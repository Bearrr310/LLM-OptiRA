...
def generate_math(problem):
    ...
    while True:  # Add loop
        ...
        # sys_prompt =read_file(args.default_prompt_position+"math_example.txt")
    ...
    user_prompt1 =read_file(args.default_prompt_position+"ask.txt")
    user_prompt1 = user_prompt.replace("$input$", math)
    sys_prompt1 =read_file(args.default_prompt_position+"ask_example.txt")
    problem1 = call_gpt_from_sys("", user_prompt1)
    with open(args.default_project_directory + "problem.txt", "w", encoding="UTF-8") as f:
        f.write(problem1)
    # print("problem is ",problem1)
